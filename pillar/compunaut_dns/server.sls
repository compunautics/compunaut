...
### SYSTEM ADDRESSES
  ...
  {%- for minion, interfaces in salt.saltutil.runner('mine.get', tgt='compunaut_salt:enabled:True', fun='network.interfaces', tgt_type='pillar').items() %}
...
### HYPERVISOR ADDRESSES
  {%- for minion, interfaces in salt.saltutil.runner('mine.get', tgt='compunaut_kvm:enabled:True', fun='network.interfaces', tgt_type='pillar').items() %}
    {%- if interfaces['br0'] is defined %}
      {%- set br0_addr = interfaces['br0']['inet'][0]['address'] %}
    {{ minion }}: {{ br0_addr }}
